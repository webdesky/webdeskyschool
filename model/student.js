...
module.exports.getSubjectList=function(table,obj, cb){

	con.connect(function(err){
		
        var que = "SELECT tbl_subject.name as subject_name from "+table.tablename+" LEFT JOIN tbl_subject ON tbl_subject.class_id="+table.tablename+".class_id WHERE "+table.tablename+".registration_id="+obj.student_id+"";		
		console.log(que)
		con.query(que, cb);
	});
}
...
module.exports.get_student_information=function(table,obj, cb){
	//console.log(obj);
	con.connect(function(err){
		var que = "SELECT "+table.tablename+".*,tbl_class.class_name,tbl_class.class_id,tbl_section.section_id,tbl_section.section_name FROM "+table.tablename+" LEFT JOIN tbl_enroll ON tbl_enroll.registration_id = "+table.tablename+".registration_id LEFT JOIN tbl_class ON tbl_class.class_id=tbl_enroll.class_id LEFT JOIN tbl_section ON tbl_section.section_id=tbl_enroll.section_id WHERE "+table.tablename+".registration_id="+obj.registration_id+"" ;
		//console.log(que);
		con.query(que, cb);
	});
}
