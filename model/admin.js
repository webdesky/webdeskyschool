...
module.exports.getStudentsformarks=function(table,obj, cb){
	...
	con.connect(function(err){
		var que = "SELECT "+table.tablename+".registration_id,tbl_registration.name  FROM "+table.tablename+" LEFT JOIN tbl_registration ON "+table.tablename+".registration_id=tbl_registration.registration_id WHERE "+table.tablename+".class_id="+obj.class_id+" AND "+table.tablename+".section_id="+obj.section_id+" AND "+table.tablename+".session_year='"+obj.session_year+"'";
        
		 //console.log('getStudentsformarks->',que); 
		con.query(que, cb);
	});
}
...
*/
module.exports.updatepassword=function(where, obj, cb){
	var que = "UPDATE tbl_userlogin SET user_password='"+(obj.login_password).replace(/'/g, "\\'")+"' WHERE registration_id = "+where.registration_id +" AND user_role = 1";
	console.log(que);
	con.connect(function(err){
		con.query(que, cb);
	});
...
module.exports.getstudentlist=function(obj,where, cb){
	    ...
	    con.connect(function(err){
	 	...
	 	//  console.log('pachvati modal ',que);
	 	con.query(que, cb);
	});
}
module.exports.getAllRegisteruser=function(obj,where, cb){

	con.connect(function(err){
        var que = "SELECT tbl_registration.*, tbl_enroll.* ,tbl_class.class_name,tbl_section.section_name FROM "+obj.tbl_enroll+"  INNER JOIN "+obj.tbl_registration+" ON "+obj.tbl_registration+".registration_id="+obj.tbl_enroll+".registration_id INNER JOIN "+obj.tbl_class+" ON "+obj.tbl_class+".class_id= "+obj.tbl_enroll+".class_id INNER JOIN "+obj.tbl_section+" ON "+obj.tbl_section+".section_id = "+obj.tbl_enroll+".section_id  WHERE "+obj.tbl_enroll+".session_year='"+where.session_year+"' AND "+obj.tbl_registration+".user_role=3 ORDER BY "+obj.tbl_registration+".name ASC" ;
		console.log('student',que); 
		con.query(que, cb);
	});
}
module.exports.getAllClassSectionRegisteruser=function(obj,where, cb){
...
module.exports.getalldaysscholler=function(obj,where, cb){
	  //
	con.connect(function(err){
	 	var que = "SELECT tbl_registration.phone,tbl_registration.parent_id, tbl_registration.name FROM "+obj.tbl_enroll+"  INNER JOIN "+obj.tbl_registration+" ON "+obj.tbl_registration+".registration_id="+obj.tbl_enroll+".registration_id WHERE  "+obj.tbl_enroll+".bonafide_status=0 AND "+obj.tbl_enroll+".session_year='"+where.session_year+"' AND "+obj.tbl_registration+".user_role=3";
	 	if(where.condition)
	 	{
	 		que += " AND "+where.condition;
	 	}
	 	if(where.hosterlsclass)
	 	{
	 		que += " AND "+where.hosterlsclass;
	 	}	
	 	 console.log('###############',que);
	 	con.query(que, cb);
	});
...
module.exports.get_subject_scheduled_exam=function(obj, where, orerby, groupby, cb){
	con.connect(function(err){
		...
		var que ="SELECT tbl_exam_schedule.exam_id,tbl_exam_schedule.class_id,tbl_exam_schedule.subject_id,tbl_exam_schedule.section_id,tbl_exam_schedule.date,tbl_exam_schedule.totalmarks,tbl_exam_master.exam_id,tbl_exam_master.exam_code,tbl_exam_master.exam_name,tbl_exam_master.session_year FROM "+obj.tbl_exam_schedule+" LEFT JOIN "+obj.tbl_exam_master+" ON "+obj.tbl_exam_master+".exam_id= "+obj.tbl_exam_schedule+".exam_id LEFT JOIN "+obj.tbl_exam_grades+" ON "+obj.tbl_exam_schedule+".exam_id= "+obj.tbl_exam_grades+".exam_id where "+obj.tbl_exam_schedule+".subject_id="+where.subject_id+" AND "+obj.tbl_exam_schedule+".class_id="+where.class_id+" AND "+obj.tbl_exam_schedule+".session_year='2018-2019' GROUP BY "+obj.tbl_exam_grades+".exam_id ORDER BY "+obj.tbl_exam_master+"."+orerby.orderby+" "+orerby.order ;
...
module.exports.find_marks_all_subject= function(table,obj,cb){
  con.connect(function(err){
    var que = "SELECT tbl_subject.name,tbl_marks.marks_obtained,tbl_marks.mark_total FROM tbl_marks LEFT JOIN tbl_subject ON tbl_marks.subject_id=tbl_subject.subject_id WHERE tbl_marks.class_id= "+obj.class_id+"  AND tbl_marks.section_id = "+obj.section_id+" AND  tbl_marks.student_id="+obj.student_id+" AND tbl_marks.exam_id="+obj.exam_id+" AND tbl_marks.year='"+obj.session_year+"'";
